...
const VerifyApplication = () => {
  ...
  return (
      <div>
        ...
        <div className="min-h-screen bg-gray-100 p-0 sm:p-12">
          <h1 className="py-3 text-black text-center font-bold text-4xl pt-8 bg-gray-100">Auntentisering</h1>
  ...
  <form onSubmit={handleSubmit}>
    ...
    <div className="relative z-0 w-full mb-5">
      ...
      <label htmlFor="name"
             className="absolute duration-300 top-3 -z-1 origin-0 text-gray-500">Brukernavn</label>
    </div>

    <div className="relative z-0 w-full mb-5">
      <input type="text" id="applicationId" placeholder=" " value={applicationId}
             onChange={(event) => setApplicationId(event.target.value)}
             className="pt-3 pb-2 block w-full px-0 mt-0 bg-transparent border-0 border-b-2 appearance-none focus:outline-none focus:ring-0 focus:border-black border-gray-200"/>
      <label htmlFor="applicationId"
          className="absolute duration-300 top-3 -z-1 origin-0 text-gray-500">Application
        ID</label>
    </div>

    <div className="relative z-0 w-full mb-5">
      <input type="text" name="name" placeholder=" " required value={tenantId}
             onChange={(event) => setTenantId(event.target.value)}
             className="pt-3 pb-2 block w-full px-0 mt-0 bg-transparent border-0 border-b-2 appearance-none focus:outline-none focus:ring-0 focus:border-black border-gray-200"/>
      <label htmlFor="tenantId"
             className="absolute duration-300 top-3 -z-1 origin-0 text-gray-500">Tenant
        ID</label>
    </div>


    <div className="relative z-0 w-full mb-5">
      <input type="password" name="name" id="clientSecret" placeholder=" " required value={clientSecret}
             onChange={(event) => setClientSecret(event.target.value)}
             className="pt-3 pb-2 block w-full px-0 mt-0 bg-transparent border-0 border-b-2 appearance-none focus:outline-none focus:ring-0 focus:border-black border-gray-200"/>
      <label htmlFor="clientSecret"
             className="absolute duration-300 top-3 -z-1 origin-0 text-gray-500">Passord/Client Secret</label>
      <span className="text-sm text-red-600 hidden" id="error">Påkrevd</span>
    </div>
